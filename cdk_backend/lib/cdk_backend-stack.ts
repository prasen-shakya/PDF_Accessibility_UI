...
export class CdkBackendStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const PDF_TO_PDF_BUCKET =
      this.node.tryGetContext("PDF_TO_PDF_BUCKET") || "Null";
    const PDF_TO_HTML_BUCKET =
      this.node.tryGetContext("PDF_TO_HTML_BUCKET") || "Null";
    ...
    if (PDF_TO_PDF_BUCKET) {
      pdfBucket = s3.Bucket.fromBucketName(
        this,
        "PDFBucket",
        PDF_TO_PDF_BUCKET
      );
    ...
    if (PDF_TO_HTML_BUCKET) {
      htmlBucket = s3.Bucket.fromBucketName(
        this,
        "HTMLBucket",
        PDF_TO_HTML_BUCKET
      );
    ...
    console.log(
      `Using main bucket for other resources: ${mainBucket.bucketName}`
    );
    ...
    const domainPrefix = `pdf-ui-auth${Math.random()
      .toString(36)
      .substring(2, 8)}`; // must be globally unique in that region
    ...
    // ------------------- Cognito: User Groups -------------------
    const defaultUsersGroup = new cognito.CfnUserPoolGroup(
      this,
      "Default_Group",
      {
    ...
    // Amazon Users Group
    const amazonUsersGroup = new cognito.CfnUserPoolGroup(
      this,
      "AmazonUsersGroup",
      {
    ...
    // Admin Users Group
    const adminUsersGroup = new cognito.CfnUserPoolGroup(
      this,
      "AdminUsersGroup",
      {
    ...
    const userPoolDomain = new cognito.CfnUserPoolDomain(
      this,
      "PDF-Accessibility-User-Pool-Domain",
      {
        domain: domainPrefix,
        userPoolId: userPool.userPoolId,
    ...
    const userPoolClient = userPool.addClient(
      "PDF-Accessibility-User-Pool-Client",
      {
    ...
    const managed_login = new cognito.CfnManagedLoginBranding(
      this,
      "MyManagedLoginBranding",
      {
        userPoolId: userPool.userPoolId,
    ...
    const checkUploadQuotaLambdaRole = new iam.Role(
      this,
      "CheckUploadQuotaLambdaRole",
      {
        assumedBy: new iam.ServicePrincipal("lambda.amazonaws.com"),
      }
    );
    ...
    checkUploadQuotaLambdaRole.addToPolicy(
      new iam.PolicyStatement({
        resources: ["*"],
        actions: [
    ...
    const checkOrIncrementQuotaFn = new lambda.Function(
      this,
      "checkOrIncrementQuotaFn",
      {
        runtime: lambda.Runtime.PYTHON_3_9,
    ...
    const updateAttributesApi = new apigateway.RestApi(
      this,
      "UpdateAttributesApi",
      {
        restApiName: "UpdateAttributesApi",
        description:
          "API to update Cognito user attributes (org, first_sign_in,country, state, city, total_file_uploaded).",
    ...
    const UpdateFirstSignIn = updateAttributesApi.root.addResource(
    ...
    UpdateFirstSignIn.addMethod(
      "POST",
      new apigateway.LambdaIntegration(updateAttributesFn),
      {
    ...
    quotaResource.addMethod(
      "POST",
      new apigateway.LambdaIntegration(checkOrIncrementQuotaFn),
      {
        authorizer: userPoolAuthorizer,
        authorizationType: apigateway.AuthorizationType.COGNITO,
      }
    ...
    if (PDF_TO_HTML_BUCKET) {
      mainBranch.addEnvironment(
        "REACT_APP_HTML_BUCKET_NAME",
        PDF_TO_HTML_BUCKET
      );
    }
    ...
    mainBranch.addEnvironment(
      "REACT_APP_UPDATE_FIRST_SIGN_IN",
      updateAttributesApi.urlForPath("/update-first-sign-in")
    );
    mainBranch.addEnvironment(
      "REACT_APP_UPLOAD_QUOTA_API",
      updateAttributesApi.urlForPath("/upload-quota")
    );

    // ------------------- Integration of UpdateAttributesGroups Lambda -------------------
    // 1. Create IAM Role
    const updateAttributesGroupsLambdaRole = new iam.Role(
      this,
      "UpdateAttributesGroupsLambdaRole",
      {
        assumedBy: new iam.ServicePrincipal("lambda.amazonaws.com"),
    ...
    updateAttributesGroupsLambdaRole.addToPolicy(
      new iam.PolicyStatement({
        effect: iam.Effect.ALLOW,
        actions: [
    ...
    const updateAttributesGroupsFn = new lambda.Function(
      this,
      "UpdateAttributesGroupsFn",
      {
        runtime: lambda.Runtime.PYTHON_3_9,
        handler: "index.handler",
    ...
    const cognitoGroupChangeRule = new events.Rule(
      this,
      "CognitoGroupChangeRule",
      {
        eventPattern: {
    ...
    cognitoGroupChangeRule.addTarget(
      new targets.LambdaFunction(updateAttributesGroupsFn)
    );
